...
async def _ask_gigachat(company_summary: str, credentials: str) -> str:
    ...
    system_prompt = (
        ...
        "Главный вопрос: МОЖНО ЛИ РАБОТАТЬ С ЭТОЙ КОМПАНИЕЙ? "
        "Формат ответа (строго!):\n"
        "1 строка: 🟢/🟡/🔴 ВЫВОД: одно предложение (можно/осторожно/нельзя)\n"
        "2 строка: пустая\n"
        "3-5 строки: ✅ Плюсы: перечисление (если есть)\n"
        "6-8 строки: ⚠️ Риски: перечисление (если есть)\n"
        "Последняя строка: 💡 РЕКОМЕНДАЦИЯ: конкретный совет (предоплата/обеспечение/избегать)\n\n"
        "Будь конкретным, без воды. Только факты из данных. Отвечай на русском. "
        "НЕ используй markdown. Используй plain text с эмодзи."
    )
