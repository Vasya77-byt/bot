"""
Счётчик предложений — хранит в SQLite (потокобезопасно).
"""
import sqlite3
import os
...
def next_proposal() -> dict:
    ...
    try:
        ...
        """, (today, today))
        conn.commit()
        row = conn.execute("SELECT total, today_count FROM counters WHERE name = 'proposals'").fetchone()
...
def get_stats() -> dict:
    """Возвращает текущую статистику без изменения счётчиков."""
    conn = sqlite3.connect(_DB_PATH)
    conn.row_factory = sqlite3.Row
    today = str(date.today())
    try:
        row = conn.execute("SELECT total, today_date, today_count FROM counters WHERE name = 'proposals'").fetchone()
        if not row:
