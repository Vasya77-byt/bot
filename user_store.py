...
# Фиксированные пакеты пополнения. Совпадают с ценами тарифов
# (500/990/2490) — чтобы юзер мог естественно «оплатить Pro»
# через топ-ап и далее автопродлевать с баланса.
# 300 — entry-level «попробовать», без привязки к тарифу.
TOPUP_PACKAGES_RUB: list = [300, 500, 990, 2490]

# Бонусы по пакетам. Мягкие — не ломают цикл авто-продления.
# Применяются по убыванию порога: первый матчинг → этот бонус.
TOPUP_BONUS_THRESHOLDS_RUB: list = [
    (2490, 10),  # 2490₽ → +10% (249₽ бонус)
    (990, 5),    # 990₽ → +5% (≈50₽ бонус)
]
